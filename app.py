...
from gerenciar_historico import resumir_historico
...
def bot(prompt):
    ...
    while True:
        try:
            ...
            print(f"Quantidade de mensagens: {len(chatbot.history)}")

            if len(chatbot.history) >= 10:
                chatbot.history = resumir_historico(
                    chatbot.history,
                    MODELO_ESCOLHIDO,
                )
                print("Histórico resumido com sucesso.")
